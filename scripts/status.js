...
process.stdin.on('end', async () => {
  try {
    ...
    if (mode === 'normal' || mode === 'mini') {
      l1 = `► CONTEXT: [${bar}] ${pctColor}${contextPct}%${RESET}`;
    } else {
      l1 = `► [${bar}] ${pctColor}${contextPct}%${RESET}`;
    }
    console.log(l1);
    console.log(sep);
    ...
    if (git.isGit) {
      ...
      if (mode === 'normal') {
        l2 += `► PWD: ${C.GIT_DIR}${dirName}${RESET} | Branch: ${C.GIT_PRIMARY}${git.branch}${RESET}`;
        ...
        if (git.totalChanged > 0 || git.untracked > 0) {
          ...
          l2 += ` | ${C.GIT_CLEAN}✓ clean${RESET}`;
        }
      } else {
        l2 += `► ${C.GIT_DIR}${dirName}${RESET} | ${C.GIT_PRIMARY}${git.branch}${RESET} `;
        if (gitIcon === '✓') l2 += `${C.GIT_CLEAN}✓${RESET}`;
        else l2 += `${C.GIT_MODIFIED}*${RESET}`;
      }
      console.log(l2);
      console.log(sep);
    } else {
      console.log(`► PWD: ${C.GIT_DIR}${dirName}${RESET}`);
      console.log(sep);
    }
  } catch (error) {
