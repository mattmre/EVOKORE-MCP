...
| #44 | open | unstable | Required fix implemented and pushed | https://github.com/mattmre/EVOKORE-MCP/pull/44#issuecomment-3998843551 |
...
| #47 | open | unstable | Required fix implemented and pushed | https://github.com/mattmre/EVOKORE-MCP/pull/47#issuecomment-3998857794 |
...
2. Merge fix-complete PRs first:
   - `#44` (includes required version authority correction)
   - `#47` (includes required sparse-rotation hardening + regression coverage)
3. Process stacked chain base-first: `#39 -> #40 -> #41 -> #42 -> #43` (rebase and revalidate at each boundary).
4. For triaged independent PRs `#45,#46,#48`, merge only after requested updates are delivered and checks are green.
5. For fully reviewed `#18` and `#29`, proceed only after conflict resolution/rebase and renewed green checks.
