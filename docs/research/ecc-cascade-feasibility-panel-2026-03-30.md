...
| 11 | Correct EVOKORE numbers | Factual correction. I verified: CLAUDE.md says 7 hooks (correct -- 8 files in `scripts/hooks/` but `fail-safe-loader.js` is shared infra). Native tools count from SkillManager + PluginManager + TelemetryManager + index.ts handler registrations needs exact inventory. The ECC plan says "5 native tools" which is wrong. |
| 12 | Re-verify ECC claims | Essential due diligence. Cannot evaluate what you have not measured. |
| 13 | Add Read to damage-control | Confirmed gap. `damage-control.js` line 138 only checks `toolName === 'Bash'` for dangerous commands. Line 163 does path extraction for all tools, and line 165 does check zero_access_paths for all tools. But the `.claude/settings.json` matcher is `Bash|Edit|Write` -- the Read tool never triggers damage-control at all. This is a real security gap. |
