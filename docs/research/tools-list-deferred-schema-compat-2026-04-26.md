...
Schema deferral (returning `tools/list` with only `name` + `description` and omitting `inputSchema`) is **spec-legal** — the official JSON Schema marks only `name` as required on a Tool, and `inputSchema` is not in the required array ([modelcontextprotocol/modelcontextprotocol schema][spec-schema]). However, **observed client behavior is far more conservative than the spec** : multiple clients silently drop tools when schema fields are unfamiliar or missing the conventional shape, and **none** of the six clients in scope advertise a `describe_tool`/`tools/get` companion call. Crucially, the official `@modelcontextprotocol/sdk` Zod schema (which most clients delegate parsing to) treats `inputSchema` as required, meaning deferred responses fail Zod validation at the SDK layer before any client code runs. The recommended Sprint 3 posture is **opt-in only, behind an env flag, default off**, with EVOKORE shipping a synthetic in-band schema fetcher (an `_evokore_describe_tool` tool exposed via the standard `tools/call` path) rather than a new RPC method.

## Spec posture

The MCP specification text describes a Tool as having `inputSchema` as a "JSON Schema defining expected parameters" but the **machine-readable schema does not list `inputSchema` in the Tool's `required` array** — only `name` is required; both `description` and `inputSchema` are optional in the spec ([spec, 2025-06-18 server/tools][spec-tools]; [schema.json][spec-schema]). The prose example in the spec always shows `inputSchema` populated, so the spec is best read as *strongly conventional* rather than *strictly required*. The spec defines no separate `tools/describe`, `tools/get`, or `tools/schema` RPC; schema is expected inline on `tools/list` ([spec-tools][spec-tools]). The list-changed notification (`notifications/tools/list_changed`) is the only documented schema-refresh signal.

> **Caveat — official SDK Zod schema is stricter than the spec.** The `@modelcontextprotocol/sdk` TypeScript SDK's `ToolSchema` Zod definition (which `ListToolsResultSchema` composes) treats `inputSchema` as a required property. Any client that runs SDK-level Zod validation on `tools/list` responses (i.e. most clients built on the SDK) will reject schema-deferred responses with a Zod validation error, dropping the entire tool list. This makes deferral risky against any SDK-using client until a confirmed source review proves a permissive parsing path. See "Open questions" below.
...
| Cline | Pass-through via MCP SDK `ListToolsResultSchema` (no local validation in `McpHub.ts`) ([cline/cline McpHub.ts][cline-hub]) | The bundled `@modelcontextprotocol/sdk` Zod `ToolSchema` lists `inputSchema` as required, so a deferred `tools/list` will fail Zod validation upstream of Cline's pass-through and the entire tool list will be dropped — even though the spec marks the field optional | None — sends tool list directly to LLM | **High risk — do not enable deferral** until a Cline-pinned SDK build confirms the Zod schema permits omission |
...
Cline is the only client in scope where source code is readable. `src/services/mcp/McpHub.ts` calls `connection.client.request({method: "tools/list"}, ListToolsResultSchema, ...)` — i.e., it delegates parsing entirely to the bundled `@modelcontextprotocol/sdk` Zod schema and does not perform additional in-Cline validation ([cline/cline McpHub.ts][cline-hub]). Arguments are also passed directly to `tools/call` without a local schema check.

**However**, the SDK's pre-validation makes deferral high-risk in practice. The official `@modelcontextprotocol/sdk` defines `ToolSchema` (the building block of `ListToolsResultSchema`) with `inputSchema` as a required Zod property. Even though the *spec*'s machine-readable JSON schema lists only `name` as required, the SDK's Zod parser will throw on a tools/list response that omits `inputSchema`, and Cline forwards SDK errors as a tool-list failure. Until either Cline ships a custom SDK build or the upstream SDK relaxes the Zod schema to mirror the spec, deferred-schema responses are expected to break the entire tool list for Cline users. This is the inverse of the original "likely safe" assessment and supersedes it.

Cline has no separate schema-fetch RPC; the LLM sees whatever Cline forwards from the list response.
...
- **Default-on candidates: none.** No client in scope is safe for default-on. The earlier "Cline likely safe" read was wrong: Cline's pass-through delegates to the official `@modelcontextprotocol/sdk` Zod parser, which lists `inputSchema` as a required property and will reject deferred responses upstream of Cline's own code. Cursor and Claude Code's documented strict-validation behavior makes default-on a guaranteed regression. Operators wanting deferral must opt in per-server *and* be on a client whose parsing path has been independently confirmed permissive.
...
- **Hard advisory: Claude Code, Cursor, Claude Desktop, Cline.** Document that operators using any of these clients should NOT enable `EVOKORE_TOOLS_LIST_DEFER_SCHEMAS`. Schema deferral against Claude Code in particular is expected to break catastrophically based on issue [#25081][cc-25081] precedent; against Cline it will fail at the upstream SDK Zod parser before Cline even sees the response.
