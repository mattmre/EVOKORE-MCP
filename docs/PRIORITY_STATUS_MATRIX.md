...
|---|---|---|---|---|
| p01 | PR review/approval playbook | done | `docs/PR_MERGE_RUNBOOK.md`, `test-ops-docs-validation.js`, `next-session.md` | Added required-check matrix and reviewer responsibility guidance. |
...
| p04 | Release flow hardening | done | `.github/workflows/release.yml`, `docs/RELEASE_FLOW.md`, `test-npm-release-flow-validation.js` | Added `origin/main` ancestry gate before publish. |
...
| p11 | Windows command resolution | done | `src/ProxyManager.ts`, `test-windows-exec-validation.js`, `test-voice-windows-docs-validation.js` | `npx.cmd` fallback and Windows guidance remain validated. |
...
| p15 | Orchestration tracking and PR slicing | in_progress | `docs/ORCHESTRATION_TRACKER.md`, `docs/RESEARCH_DECISIONS_LOG.md`, `docs/session-logs/session-2026-02-24-agentic-orchestration-refresh.md` | Session tracking is updated; PR creation/slicing remains open. |
